...
function sha256Fallback(data: ArrayBuffer): string {
  const K = [
  ...
  view.setUint32(withOne.length - 4, bitLength >>> 0);
  view.setUint32(withOne.length - 8, Math.floor(bitLength / 0x100000000));
